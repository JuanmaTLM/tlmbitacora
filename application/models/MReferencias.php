...
*/
		...
		$file_url = "assets/DCTOSREFERENCIAS/" . $cve;
		$url_insert = $baseUrl .$file_url;
		...
		$file_targetBL = $file_url . $nameBL;
		$file_targetFactura = $file_url . $nameFactura;
		$file_targetListaEmp = $file_url . $nameListaEmp;
		...
		$query = "CALL insertBlFaEm ('" .$cve . "','" . $nameBL . "','" . $file_targetBL. "','" . $nameFactura . "','" . $$file_targetFactura . "','" . $nameListaEmp . "','" . $file_targetListaEmp . "')";
	...
	public function newGasto(){
		...
		if(isset($_POST['txtConcept'])){
			...
			if(isset($_FILES['dctoPago'])){
				...
				$url = dirname(__FILE__);
				$baseUrl = substr($url, 0, -18);
				$file_url = "assets/DCTOSREFERENCIAS/".$cveRef ."/" .$data['cveCuentaGastos']."/";
				$url_insert = $baseUrl .$file_url;
				...
				if(move_uploaded_file($url_temp, $url_target)){
					$query ="INSERT INTO listgastos(txtAuthor,fk_eIdCtaGastos, txtGasto, txtValor, txtFileUrl, txtfileName,fePago, feCreacion, feModificacion,bPagado) VALUES ('".$user."',".$data['eIdCuentaGastos'].",'".$data['txtConcept']."',".$data['flPrice'].",'".$file_url."','".$fileName."','".$data['fePago']."','".$fechaHora."','".$fechaHora."',1)";
					$result = $this->db->query($query);
					if($result){
	...
	public function newDeposito(){
		...
		if(isset($_POST['txtDConcept'])){
			...
			if(isset($_FILES['dctoDep'])){
				...
				$url = dirname(__FILE__);
				$baseUrl = substr($url, 0, -18);
				$file_url = "assets/DCTOSREFERENCIAS/".$cveRef ."/" .$data['cveCuentaGasto']."/".$data['txtDConcept']."s/";
				$url_insert = $baseUrl .$file_url;
				$url_insert = trim(str_replace('\\', '/', $url_insert));
				$url_target = $url_insert . $fileName;
				if (!file_exists($url_insert)) {
	...
	*/
	...
	public function vwReference(){
		$id = $_GET['idRef'];
		$query = "SELECT * FROM vwReferenciaCliente WHERE eIdReferencia =". $id;
		$result = $this->db->query($query);
		if($result){
			return $result->result_array();
		}else{
	...
	public function vwDocumentsRef(){
		$id = $_GET['idRef'];
		$query = "SELECT * FROM vwDocumentsRefs WHERE eIdReferencia =". $id;
		$result = $this->db->query($query);
		if($result){
			return $result->result_array();
		}else{
	...
	public function updateRef($data){
		$fechaCorta = date("Y-m-d");
		$fechaHora = date("Y-m-d h:i:s");
		$query = "CALL updateReference (".$data['eIdReferencia'].",'".$data['txtCliente']."','".$data['txtReferencia']."','".$data['txtETA']."','".$data['txtNaviera']."','".$data['txtVessel']."','".$data['txtVoyaje']."','".$data['txtTerminal']."','".$fechaHora."');";
		$result = $this->db->query($query);
		if($result){
			return true;
		}
